...
export default function VideoPlayer({
  ...
  // Reset playback start state when episode changes
  useEffect(() => {
    setHasStarted(false);
    setIsPlaying(false);
    setCurrentTime(0);
  }, [episodeNum, server.id]);
  ...
  // Main start handler: hides poster and auto-plays video immediately
    ...
    if (videoRef.current) {
      ...
      if (playPromise !== undefined) {
        playPromise
          ...
          .catch(() => {
            // Autoplay fallback if muted or blocked
            setIsPlaying(true);
            setIsLoading(false);
  ...
  const togglePlay = useCallback(() => {
    ...
    setTimeout(() => setCenterAnimation(null), 600);
  }, [hasStarted, isPlaying, duration, handleStartPlayback]);
  ...
  useEffect(() => {
    ...
    if (
      hasStarted &&
      isPlaying &&
      (!videoRef.current || videoRef.current.paused || isNaN(videoRef.current.duration))
    ) {
      timer = setInterval(() => {
        setCurrentTime((prev) => {
  ...
  }, [hasStarted, isPlaying, duration, onEpisodeEnded, onSaveProgress]);
  ...
  return (
    <div
      ...
      className={`relative w-full aspect-video rounded-2xl overflow-hidden shadow-2xl transition-all duration-300 select-none bg-black ${
    ...
    >
      {/* 1. EMBED SERVER (Official Anime Studio Embed / YouTube) */}
      {server.type === "embed" && hasStarted && (
        <iframe
          src={server.url}
      ...
      {/* 3. POSTER OVERLAY WITH GIANT PLAY BUTTON (Disappears completely when clicked) */}
      {!hasStarted && (
        <div
          onClick={handleStartPlayback}
          className="absolute inset-0 z-40 flex flex-col items-center justify-center cursor-pointer group bg-black/70 backdrop-blur-xs transition-opacity duration-300"
        >
              ...
              {animeImage && (
                <Image
                  ...
                  className="object-cover opacity-35 filter blur-xs group-hover:scale-105 transition-transform duration-700 pointer-events-none"
                />
              )}
              ...
              {/* Center Content: Title Badge + Giant Glowing Play Button */}
              <div className="relative z-10 flex flex-col items-center gap-4 text-center px-4">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-600/80 backdrop-blur-md text-white text-xs font-bold shadow-lg shadow-violet-600/30 border border-violet-400/30">
                ...
                <div className="relative mt-2">
                  <div className="absolute -inset-3 rounded-full bg-violet-600/40 blur-xl group-hover:bg-violet-500/60 group-hover:scale-125 transition-all duration-500 animate-pulse" />
                  <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-tr from-violet-600 to-indigo-500 flex items-center justify-center shadow-2xl shadow-violet-600/60 border-2 border-white/40 group-hover:scale-110 group-active:scale-95 transition-all duration-300">
                    <Play className="w-9 h-9 sm:w-11 sm:h-11 text-white fill-white ml-1.5" />
                  </div>
                </div>

                <p className="text-xs sm:text-sm font-semibold text-slate-300/90 group-hover:text-white transition-colors">
          ...
          {/* 5. CENTER PLAY/PAUSE ANIMATED POPUP */}
          ...
          {showResumeBanner && (
            <div className="absolute top-4 left-4 right-4 z-50 flex items-center justify-between p-3 rounded-xl bg-black/85 backdrop-blur-md border border-violet-500/50 shadow-lg shadow-violet-500/20 animate-fade-in">
          ...
          {/* 7. QUICK SKIP INTRO / OUTRO BUTTONS */}
          {hasStarted && (
            <div
              ...
              <div className="flex items-center justify-between text-white">
                ...
                <div className="flex items-center gap-2 relative">
                  ...
                  <div className="relative">
                    ...
                    {showSettings && (
                      <div className="absolute bottom-12 right-0 w-52 rounded-xl p-2 bg-black/90 backdrop-blur-xl border border-white/20 shadow-2xl text-xs z-50 animate-fade-in">
                        {settingsTab === "main" && (
